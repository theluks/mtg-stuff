...
for card in cards:
    ...
    if token == False:
        ...
        card_dict = {'quantity': card_amount, 'name': card_name, 'setcode': setcode.upper(), 'condition': card_condition, 'language': card_language, 'foil': card_foil, 'collnumber': card_collno}
        card_data.append(card_dict)
    else:
        print('Token found, "' + card_name + '", not writing to file')

#Write to csv file
with open('archidekt_data.csv', mode='w', newline='') as file:
    writer = csv.DictWriter(file, delimiter=',',quoting=csv.QUOTE_MINIMAL, fieldnames=['quantity', 'name', 'setcode', 'collnumber', 'condition', 'language', 'foil'])
    writer.writeheader()
